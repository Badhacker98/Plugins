...
from TelethonPbx.DB import gmute_sql as gsql
from TelethonPbx.DB.gban_sql import all_gbanned, gbaner, is_gbanned, ungbaner
...
async def _(event):
    ...
    yup = 0
    sed = 0
    Pbx = await eor(event, "GPromote in action...")
    if event.is_private:
        user = event.chat
        rank = event.text[6:]
    elif event.is_group:
        ...
        return await parse_error(Pbx, "Can be used in Group and PMs only.")
    if II_BAD_BBY_II == user:
        return await parse_error(Pbx, "Cant promote self.")
    if not rank:
        rank = "ǟɖʍɨռ"
    if user:
        total_ch = [
...
async def _(event):
    ...
    sed = 0
    rank = "ǟɖʍɨռ"
    Pbx = await eor(event, "GDemote in action...")
    if event.is_private:
        user = event.chat
    elif event.is_group:
        user, _ = await get_full_user(event)
    else:
        return await parse_error(Pbx, "can be used in groups or PMs only!")
    if II_BAD_BBY_II == user:
        return await parse_error(Pbx, "Can't demote self.")
    if user:
        total_ch = [
            x.entity.id
            for x in await event.client.get_dialogs()
    ...
    await Pbx.edit(f"<b><i>🔥 GDemote Completed !!</b></i> \n\n{text_to_send}", parse_mode="HTML")
    await event.client.send_message(
        Config.LOGGER_ID,
        f"#GDEMOTE \n\n{text_to_send}",
        parse_mode="HTML",
...
@Pbx_cmd(pattern="gban(?:\s|$)([\s\S]*)")
async def _(event):
    reason = ""
    II_BAD_BBY_II, _, Pbx_mention = await client_id(event)
    Pbx = await eor(event, f"`Gban in action...`")
    ...
    if event.reply_to_msg_id:
        ...
        return await parse_error(Pbx, "No user mentioned.")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == II_BAD_BBY_II:
        return await eod(Pbx, "🥴 **It's simply not possible.**")
    if str(userid) in DEVLIST:
        return await eod(Pbx, "😑 **It's simply not possible.**")
    if is_gbanned(userid):
        return await eod(Pbx, "This user is already gbanned and added to my **Gban Watch!!**")
    ...
    gmsg = f"✌️🏻 {Pbx_mention} **𝐇ᴀᴛʜ 𝐌ᴇʀᴀ 𝐆ᴜɴ🔫𝐏ᴀʀ [{name}](tg://user?id={userid}) 𝐉ᴀɪsᴇ 𝐊ᴏ💪🏻𝐑ᴀᴋʜ 𝐓ᴀ 𝐇ᴜ 𝐋ᴜɴᴅ🖕🏻𝐏ᴀʀ 😈**\n\n❌ Added to Gban Watch!!\n**📃 Ma chudva [{name}](tg://user?id={userid})Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**📝 Reason :**  `{reason}`"
...
@Pbx_cmd(pattern="ungban(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, Pbx_mention = await client_id(event)
    Pbx = await eor(event, "`Ungban in action...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
    ...
    if not is_gbanned(userid):
        return await eod(Pbx, "`User is not gbanned.`")
    ...
    ungbaner(userid)
    ogmsg = f"**❌ Victim:** [{name}](tg://user?id={userid}) \n**❌ Chats:** `{chats}` \n**❌ UnGban By:** {Pbx_mention}\n\n**❌ User removed from Gban Watch!!**"
    await Pbx.edit(f"__**🔑 Chil baby chil 🔓**__ \n\n{ogmsg}")
    await event.client.send_message(
        Config.LOGGER_ID,
        f"#UNGBAN \n\n{ogmsg}",
    )
...
@Pbx_cmd(pattern="listgban$")
async def already(event):
    Pbx = await eor(event, "`Fetching Gbanned users...`")
...
@Pbx_cmd(pattern="gkick(?:\s|$)([\s\S]*)")
async def gkick(event):
    reason = ""
    II_BAD_BBY_II, _, Pbx_mention = await client_id(event)
    Pbx = await eor(event, "`GKick in action...`")
    reply = await event.get_reply_message()
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
        try:
...
@Pbx_cmd(pattern="gmute(?:\s|$)([\s\S]*)")
async def gm(event):
    II_BAD_BBY_II, _, Pbx_mention = await client_id(event)
    Pbx = await eor(event, "`GMute in action...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
    ...
    if gsql.is_gmuted(userid, "gmute"):
        return await eod(Pbx, "This user is already Gmuted.")
    
    ogmsg = f"**📍 Victim:** [{name}](tg://user?id={userid}) \n**📍 GMute By:** {Pbx_mention}"
    try:
        gsql.gmute(userid, "gmute")
        if Config.ABUSE == "ON":
            await event.client.send_message(event.chat_id, f"😈𝐉αв {Pbx_mention} 𝐑αѕтє 𝐌є 𝐉α 𝐑αнα 𝐓нα 𝐑αѕтє🛣 𝐌є 𝐏∂є 𝐓нєн 𝐑σ∂є🪨 𝐂нυρ🤫 𝐊αя [{name}](tg://user?id={userid}) 𝐁нєи 𝐊є 𝐋σ∂є🚫", file=shhh, reply_to=reply)
            await Pbx.delete()
        else:
            await Pbx.edit(f"__**🥵 Chup kar Madarchod 👅**__ \n\n{ogmsg}")
        await event.client.send_message(
            Config.LOGGER_ID,
            f"#GMUTE \n\n{ogmsg}",
        )
...
@Pbx_cmd(pattern="ungmute(?:\s|$)([\s\S]*)")
async def endgmute(event):
    _, _, Pbx_mention = await client_id(event)
    Pbx = await eor(event, "`UnGmute in action...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        userid = await get_user_id(event, event.text.split(" ", 2)[1])
    ...
    if not gsql.is_gmuted(userid, "gmute"):
        return await eod(Pbx, "User not found in Gmute DB.")
    try:
        gsql.ungmute(userid, "gmute")
        ogmsg = f"**📍 Victim:** [{name}](tg://user?id={userid}) \n**📍 UnGmute By:** {Pbx_mention}"
        await Pbx.edit(f"__**🥵 Ab bol na Madarchod👅**__ \n\n{ogmsg}")
        await event.client.send_message(
            Config.LOGGER_ID,
            f"#UNGMUTE \n\n{ogmsg}",
        )
