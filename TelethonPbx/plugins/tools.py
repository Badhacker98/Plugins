...
@Pbx_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
    Pbxevent = await eor(event, "...")
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    ...
    try:
        ...
        await Pbxevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await Pbxevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await Pbxevent.delete()


@Pbx_cmd(pattern="makeqr(?: |$)([\s\S]*)")
...
@Pbx_cmd(pattern="calendar(?:\s|$)([\s\S]*)")
async def _(event):
...
@Pbx_cmd(pattern="currency(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if lists[1].strip().isdigit():
        ...
        return await parse_error(event, f"__Give proper command:__ \n__Ex:__`{hl}currency 10 usd inr`", False)
    Pbx = await eor(event, f"**Converting currency:** \n__From:__ `{base}` \n__To:__ `{into}` \n__Amount:__ `{amnt}`")
    ...
    try:
        if data['result'] == 'success':
            ...
            await Pbx.edit(f"**Currency Converted !!** \n\n{output}")
        else:
            await eod(Pbx, "Something went wrong. Try again later!")
    except Exception as e:
        await parse_error(Pbx, e)


@Pbx_cmd(pattern="currencies$")
async def currencylist(event):
    if not Config.CURRENCY_API:
        return await parse_error(event, "`CURRENCY_API` __not configured.__", False)
    Pbx = await eor(event, "Fetching supportred currencies lists ...")
    ...
    try:
        ...
        output += "\n<img src='https://telegra.ph/file/858f2903b5cb394b0a018.jpg'/>"
        link = await telegraph_paste("Currency List For PbxBot", output)
        await Pbx.edit(
...
@Pbx_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    Pbxevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await Pbxevent.edit(output_str, link_preview=True)
    else:
        await eod(Pbxevent, "xkcd n.{} not found!".format(xkcd_id))


@Pbx_cmd(pattern="dns(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("tools").add_command(
    ...
    "currency", "<amount> <from> <to>", "Currency converter for PbxBot", ".currency 10 usd inr"
).add_command(
