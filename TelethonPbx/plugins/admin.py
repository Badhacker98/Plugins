...
from TelethonPbx.DB.mute_sql import is_muted, mute, unmute
...
async def promote(event):
    ...
    Pbxevent = await eor(event, "`Promoting User...`")
    ...
    try:
        ...
        return await parse_error(Pbxevent, NO_PERM)
    await event.client.send_message(
        Config.LOGGER_ID,
        "#PROMOTE\n"
...
async def demote(event):
    ...
    if not admin and not creator:
        return await parse_error(event, NO_ADMIN)
    Pbxevent = await eor(event, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        return await parse_error(Pbxevent, NO_PERM)
    await Pbxevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{event.chat.title}`")
    await event.client.send_message(
        Config.LOGGER_ID,
        "#DEMOTE\n"
        f"\n**USER:** [{user.first_name}](tg://user?id={user.id})"
...
@Pbx_cmd(pattern="mute(?:\s|$)([\s\S]*)")
async def muth(event):
    x = await client_id(event)
    II_BAD_BBY_II = x[0]
    if event.is_private:
        Pbx = await eor(event, "**Enough of your bullshit  !!**")
        await event.get_reply_message()
        if is_muted(event.chat_id, event.chat_id):
            return await eod(Pbx, "User is already muted here 🥴")
        if event.chat_id == II_BAD_BBY_II:
            return await parse_error(Pbx, "You can't mute yourself !")
        try:
            mute(event.chat_id, event.chat_id)
            await eod(Pbx, "**Muted this user !**")
        except Exception as e:
            return await parse_error(Pbx, e)
    elif event.is_group:
        Pbx = await eor(event, "`Muting...`")
        ...
        if event.reply_to_msg_id:
            ...
            if input_str.isdigit():
                try:
                    ...
                    return await parse_error(Pbx, ve)
            else:
                userid = (await event.client.get_entity(input_str)).id
                name = (await event.client.get_entity(userid)).first_name
        else:
            return await parse_error(Pbx, "I need an user to mute !!")
        if userid == II_BAD_BBY_II:
            return await parse_error(Pbx, "You can't mute yourself !")
        if str(userid) in DEVLIST:
            return await parse_error(Pbx, "Cant mute my developers !")
        if II_BAD_BBY_II not in admin_:
            return await parse_error(Pbx, NO_PERM)
        if userid in admin_:
            if is_muted(userid, event.chat_id):
                return await eod(Pbx, "Admin already muted ♪～(´ε｀ )")
            try:
...
@Pbx_cmd(pattern="unmute(?:\s|$)([\s\S]*)")
async def nomuth(event):
    x = await client_id(event)
    II_BAD_BBY_II = x[0]
    Pbx = await eor(event, "`Unmuting ...`")
    if event.is_private:
        if not is_muted(event.chat_id, event.chat_id):
            return await eod(Pbx, "Not even muted !!")
        try:
            unmute(event.chat_id, event.chat_id)
            await eod(Pbx, "User unmuted successfully !")
        except Exception as e:
            await parse_error(Pbx, e)
    elif event.is_group:
        input_str = event.pattern_match.group(1)
        ...
        if event.reply_to_msg_id:
            ...
            return await parse_error(Pbx, "I need a user to unmute!!")
        if II_BAD_BBY_II not in admin_:
            return await parse_error(Pbx, NO_PERM)
        if userid in admin_:
...
@Pbx_cmd(pattern="ban(?:\s|$)([\s\S]*)")
@errors_handler
async def ban(event):
    Pbxevent = await eor(event, "`Banning Nigga...`")
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
    ...
    if not user:
        return await parse_error(Pbxevent, "`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await eod(Pbxevent, "**Say again? Ban my creator??**")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        await parse_error(Pbxevent, NO_PERM)
        return
    try:
        reply = await event.get_reply_message()
        if reply:
...
async def nothanos(event):
    ...
    if not admin and not creator:
        await parse_error(event, NO_ADMIN)
        return
    Pbxevent = await eor(event, "`Unbanning...`")
    user, _ = await get_user_from_event(event)
    if not user:
        return
    try:
        ...
        await parse_error(Pbxevent, "Invalid UserId!! Please Recheck it!!")


@Pbx_cmd(pattern="pin(?:\s|$)([\s\S]*)")
@errors_handler
async def pin(event):
    chat = await event.get_chat()
...
async def kick(event):
    ...
    Pbxevent = await eor(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await parse_error(Pbxevent, e)
        return
    if reason:
        await Pbxevent.edit(
...
async def rm_deletedacc(event):
    ...
    if action == "clean":
        ...
        if not admin and not creator:
            return await parse_error(event, NO_ADMIN)
        Pbx = await eor(event, "__🧹 Purging Zombies from here ...__")
        del_u = 0
        del_a = 0
...
#                     await sleep(0.5)
        ...
        Pbx = await eor(event, "**Searching For Zombies...**")
        async for user in event.client.iter_participants(event.chat_id):
            if user.deleted:
                del_u += 1
                await sleep(0.5)
