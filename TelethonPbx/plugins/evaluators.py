...
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        ...
        try:
            ...
            result = _out_
        Pbxuser = await event.client.get_me()
        if Pbxuser.username:
            curruser = Pbxuser.username
        else:
            curruser = "@ll_THE_BAD_BOT_ll"
        uid = os.geteuid()
        if uid == 0:
...
@Pbx_cmd(pattern="eval(?:\s|$)([\s\S]*)")
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        cmd = "".join(event.text.split(maxsplit=1)[1:])
        if not cmd:
            return await eod(event, "`What should i run ?..`")
        Pbxevent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec", "<command>", "Execute a Terminal command on PbxBot server and shows details"
).add_command(
