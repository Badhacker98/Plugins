...
from TelethonPbx.helpers.pasters import pasty
...
# delete timeout
async def delete_Pbx(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 10
    if event.sender_id in Config.SUDO_USERS:
        ...
        Pbxevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await Pbxevent.delete()


async def parse_error(event, error, auto_parse=True, delete=True, time=10):
    if delete:
        if auto_parse:
            await delete_Pbx(event, f"**ERROR !!** \n\n`{error}`", time)
        else:
            await delete_Pbx(event, f"**ERROR !!** \n\n{error}", time)
    else:
        if auto_parse:
