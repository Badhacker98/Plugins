...
@Pbx_cmd(pattern="anime(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        return await eor(event, "Please give anime name to search on Anilist.")
    Pbx = await eor(event, f"__Searching for__ `{query}` __on Anilist.__")
    qdb = rand_key()
    ANIME_DB[qdb] = query
    result = await get_anilist(qdb, 1)
    if len(result) == 1:
        return await Pbx.edit(result[0])
...
@Pbx_cmd(pattern="manga(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        await eor(event, "Please give manga name to search..")
    Pbx = await eor(event, f"__Searching for__ `{query}` ...")
    qdb = rand_key()
    MANGA_DB[qdb] = query
    ...
    try:
        ...
        await Pbx.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)


@Pbx_cmd(pattern="character(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[11:]
    if query == "":
    ...
    try:
        ...
        await Pbx.delete(cap_text)
    if os.path.exists(img):
        os.remove(img)


@Pbx_cmd(pattern="fillers(?:\s|$)([\s\S]*)")
async def canon(event):
    Pbx = event.text[9:]
    if Pbx == "":
        return await eor(event, "`Give anime name to search filler episodes.`")
    nub = await eor(event, f"Searching Filler Episodes For `{Pbx}`")
    hel_ = search_filler(Pbx)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{Pbx}`")
    ...
    for i in list_:
        ...
        Pbxbot += f"• [{i}]({paste})\n"
    await nub.edit(Pbxbot)


@Pbx_cmd(pattern="airing(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[8:]
    Pbx = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(Pbx, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
...
@Pbx_cmd(pattern="aniuser(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[9:]
    Pbx = await eor(event, "Searching user's Anilist Stats...")
    if query == "":
        return await Pbx.edit("No user found. Give anilist username.")
    qry = {"search": query}
    result = await get_user(qry)
    if len(result) == 1:
        return await eod(Pbx, result[0])
    pic, msg = result
    try:
        await event.client.send_file(
            event.chat_id,
...
CmdHelp("anime").add_command(
    ...
    "aniuser", "<anilist username>", "Searches for the Anilist Stats of the given user.", "aniuser Badboy"
).add_command(
