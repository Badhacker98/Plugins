...
@Pbx_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)")
async def variable(Pbx):
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await parse_error(Pbx, "`HEROKU_APP_NAME` is not configured.", False)
    exe = Pbx.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(Pbx, "Getting Variable Info...")
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            xvar = Pbx.pattern_match.group(2).split()[0]
            variable = xvar.upper()
            ...
            if variable in ("PBXBOT_SESSION", "BOT_TOKEN", "HEROKU_API_KEY", "INSTAGRAM_SESSION"):
                if Config.ABUSE == "ON":
                    await event.client.send_file(Pbx.chat_id, cjb, caption=cap)
                    await event.delete()
        ...
        event = await eor(Pbx, "Setting Heroku Variable...")
        xvar = Pbx.pattern_match.group(2)
        if not xvar:
            return await eod(event, f"`{hl}set var <Var Name> <Value>`")
        variable = xvar.upper()
        value = Pbx.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = Pbx.pattern_match.group(2).split()[1]
            except IndexError:
                return await eod(event, f"`{hl}set var <Var Name> <Value>`")
        if variable in db_config:
            return await eod(
        ...
        event = await eor(Pbx, "Getting info to delete Variable")
        try:
            xvar = Pbx.pattern_match.group(2).split()[0]
        except IndexError:
...
@Pbx_cmd(pattern="usage$")
async def dyno_usage(Pbx):
    event = await eor(Pbx, "`Processing...`")
    useragent = (
