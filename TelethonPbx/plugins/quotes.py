...
@Pbx_cmd(pattern="love$")
async def love(e):
    txt = choice(LOVESTR)
    await eor(e, txt)


@Pbx_cmd(pattern="dhoka$")
async def katgya(e):
    txt = choice(DHOKA)
    await eor(e, txt)


@Pbx_cmd(pattern="metoo$")
async def metoo(e):
    txt = choice(METOOSTR)
    await eor(e, txt)


@Pbx_cmd(pattern="gdnoon$")
async def noon(e):
    txt = choice(GDNOON)
    await eor(e, txt)


@Pbx_cmd(pattern="chase$")
async def police(e):
    txt = choice(CHASE_STR)
    await eor(e, txt)


@Pbx_cmd(pattern="congo$")
async def Sahih(e):
...
@Pbx_cmd(pattern="gdbye$")
async def bhago(e):
    txt = choice(BYESTR)
    await eor(e, txt)


@Pbx_cmd(pattern="gdnyt$")
async def night(e):
    txt = choice(GDNIGHT)
    await eor(e, txt)


@Pbx_cmd(pattern="gdmng$")
async def morning(e):
    txt = choice(GDMORNING)
    await eor(e, txt)


@Pbx_cmd(pattern="quote(?:\s|$)([\s\S]*)")
async def quote_search(event):
    ...
    if response is not None:
        await Pbx.edit(f"`{response['text']}`")
    else:
        await parse_error(Pbx, "No quote found! Try again later.")
