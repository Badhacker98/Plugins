...
@Pbx_cmd(pattern="lock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[6:]
    cid = await client_id(event)
    Pbx_mention = cid[2]
    Pbx = await eor(event, f"Trying to lock `{text}`")
    if text == "":
        return await parse_error(Pbx, "Nothing given to lock in chat.")
    locker = lock(text)
    if not locker:
        return await eod(
            Pbx,
    ...
    await event.client.send_file(
        event.chat_id,
        restlo,
        caption=f"{Pbx_mention} Locked `{text}` \n__Cause its Rest Time !!__",
    )
    await Pbx.delete()


@Pbx_cmd(pattern="unlock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[8:]
    cid = await client_id(event)
    Pbx_mention = cid[2]
    Pbx = await eor(event, f"Trying to unlock `{text}`")
    if text == "":
        return await parse_error(Pbx, "Nothing given to unlock in chat.")
    ...
    await event.client.send_file(
        event.chat_id, shuru, caption=f"**{Pbx_mention} unlocked** `{text}`"
    )
    await Pbx.delete()


@Pbx_cmd(pattern="ltype$")
async def _(event):
    await eor(
        event,
