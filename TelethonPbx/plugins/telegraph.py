...
async def _(event):
    ...
    Xabhish3k, PBX_USER, pbx_mention = await client_id(event)
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            await pbx.edit(f"Downloaded to  `{downloaded_file_name}`  in  `{ms}`  seconds. \nMaking Telegraph Link.....")
            ...
            try:
                ...
                await eod(pbx, "ERROR: " + str(exc), 8)
                os.remove(downloaded_file_name)
            else:
                end = datetime.datetime.now()
                ms_two = (end - start).seconds
                os.remove(downloaded_file_name)
                await pbx.edit("✓ **[File uploaded to telegraph](https://telegra.ph{})** \n✓ **Time Taken :-** `{}` secs \n✓ **By :- {}** \n✓  `https://telegra.ph{}`".format(
                        media_urls[0], (ms + ms_two), pbx_mention, media_urls[0],
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            pbxboy = f"https://telegra.ph/{response['path']}"
            await pbx.edit(f"✓ **[Pasted to telegraph]({pbxboy})** \n✓ **Time Taken :-** `{ms}` secs\n✓** By :**  {pbx_mention} \n✓  `{pbxboy}`", link_preview=True)
    else:
