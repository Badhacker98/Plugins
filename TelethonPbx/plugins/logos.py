...
async def logo(event):
    ...
    lists = text.split(" ", 1)
    if len(lists) == 1:
        return await parse_error(Pbx, "Give some text to make Logo")
    ...
    if reply and reply.media and reply.media.photo:
        ...
        await Pbx.edit("__Downloaded replied photo... starting to make logo__")
    else:
        _unsp = await unsplash(_type, 1)
        _bg = requests.get(_unsp[0])
        with open("temp_bg.jpg", "wb") as file:
            file.write(_bg.content)
        await Pbx.edit(f"__Downloaded__ `{_type}` __background... starting to make logo__")
...
CmdHelp("logos").add_command(
    "logo", "-{type} {logo text}", "Makes a logo with the given text. If replied to a picture makes logo on that else gets random BG.", f"logo Pbxbot \n{hl}logo-car PbxBot \n{hl}logo-anime PbxBot \netc..."
).add_info(
    "Logo Maker."
).add_warning(
    "✅ Harmless Module."
