...
@Pbx_cmd(pattern="offline$")
async def _(event):
    II_BAD_BBY_II, PBX_USER, _ = await client_id(event)
    user = await event.client.get_entity(II_BAD_BBY_II)
    if PBX_USER.startswith("[ • OFFLINE • ]"):
        return await eod(event, "**Already in Offline Mode.**")
    Pbx = await eor(event, "**Changing Profile to Offline...**")
    ...
    if photo:
        ...
        try:
            ...
            await eor(Pbx, "**Changed profile to OffLine.**")
    try:
    ...
    try:
        ...
        await eor(Pbx, result)
    except Exception as e:
        await parse_error(Pbx, e)


@Pbx_cmd(pattern="online$")
async def _(event):
    II_BAD_BBY_II, PBX_USER, _ = await client_id(event)
    user = await event.client.get_entity(II_BAD_BBY_II)
    if PBX_USER.startswith("[ • OFFLINE • ]"):
        Pbx = await eor(event, "**Changing Profile to Online...**")
    else:
    ...
    if photo:
        ...
        try:
            ...
            await eor(Pbx, "**Changed profile to Online.**")
    try:
        os.remove("donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
@Pbx_cmd(pattern="ppic$")
async def _(event):
    reply = await event.get_reply_message()
    Pbx = await eor(event, "Changing profile picture ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            ...
            try:
                ...
                await eod(Pbx, "**📍 Profile picture changed successfully.**")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.warn(str(e))
...
@Pbx_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await Pbx.edit(result, parse_mode="HTML")


@Pbx_cmd(pattern="delpfp(?:\s|$)([\s\S]*)")
async def remove_profilepic(event):
    ...
    pfplist = await event.client(
        GetUserPhotosRequest(user_id=II_BAD_BBY_II, offset=0, max_id=0, limit=lim)
    )
    input_photos = []
...
@Pbx_cmd(pattern="myusernames$")
async def _(event):
    _, _, Pbx_mention = await client_id(event)
    result = await event.client(GetAdminedPublicChannelsRequest())
    output_str = f"**Usernames reserved by:** {Pbx_mention}\n\n"
    for channel_obj in result.chats:
        output_str += f"• {channel_obj.title} ~ @{channel_obj.username} \n"
    await eor(event, output_str)


@Pbx_cmd(pattern="stats$")
async def stats(event):
    Pbx = await eor(event, "`Collecting stats...`")
    ...
    response = f"<b><i><u>♛ Stats for {Pbx_mention} ♛</b></i></u>\n\n"
    ...
    await Pbx.edit(response, parse_mode="HTML", link_preview=False)
