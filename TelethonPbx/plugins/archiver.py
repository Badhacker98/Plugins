...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await eod(Pbx, downloaded_file_name)
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            directory_name = downloaded_file_name
            await Pbx.edit("Finished downloading to my local")
...
@Pbx_cmd(pattern="unzip$")
async def _(event):
    Pbx = await eor(event, "Processing ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if not event.reply_to_msg_id:
        return await parse_error(Pbx, "Reply to zip file!")
    else:
        ...
        try:
            ...
            await Pbx.edit("Stored the zip to `{}` in {} seconds.".format(downloaded_file_name, ms))
        extracted = "./extracted"
        with zipfile.ZipFile(downloaded_file_name, "r") as zip_ref:
            zip_ref.extractall(extracted)
        filename = sorted(get_lst_of_files(extracted, []))
        await Pbx.edit("Unzipping now")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            ms = (end - start).seconds
            await Pbx.edit(f"Stored the tar to `{downloaded_file_name}` in {ms} seconds.")
