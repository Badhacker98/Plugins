...
async def download_video(event):
    ...
    Pbx = await eor(event, "`Preparing to download...`")
    if type_ == "a":
        opts = song_opts
        video = False
        song = True
    ...
    if song:
        try:
            await Pbx.edit("**Fetching YT link...**")
            ...
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, Pbx, c_time, upload_txt, f"{ytdl_data['title']}.mp3"
                    )
                ),
            )
        ...
        try:
            ...
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, Pbx, c_time, upload_txt, f"{vid_file['title']}.mp4"
                    )
                ),
            )
            os.remove(file_)
...
async def hmm(event):
    ...
    try:
        results = json.loads(Pbx_YTS(query, max_results=7).to_json())
    except KeyError:
    ...
    await Pbx.edit(output, link_preview=False)
