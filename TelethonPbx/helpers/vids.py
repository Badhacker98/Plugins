...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            Pbxresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            Pbxfile = await event.client.download_media(Pbxresponse, "./temp")
            return await unzip(Pbxfile)
