...
<b>⚡ <a href='https://t.me/ll_THE_BAD_BOT_ll'>From DataBase of 🕊️⃝‌Pbx ❤️ᥫ᭡፝֟፝֟</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@Pbx_cmd(pattern="chatinfo(?:\s|$)([\s\S]*)")
async def info(event):
    Pbx = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await Pbx.edit(caption, parse_mode="HTML")
    except Exception as e:
        print("Exception:", e)
        await parse_error(Pbx, "An unexpected error has occurred.")
    return
...
@Pbx_cmd(pattern="id$")
async def _(event):
    Pbx = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await Pbx.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
