...
@Pbx_cmd(pattern="kang(?:\s|$)([\s\S]*)")
async def kang(event):
    Pbx = await eor(event, "__**Starting sticker kang process ...**__")
    ...
    II_BAD_BBY_II, PBX_USER, _ = await client_id(event)
    nick = f"@{user.username}" if user.username else PBX_USER
    name = user.username if user.username else II_BAD_BBY_II
    custompack = gvarstat("STICKER_PACKNAME")
    packname = f"PbxBot_{name}_{Sticker.pack}"
    packnick = f"{custompack}" if custompack else f"{nick}'s 🕊️⃝‌ᴘʙx ❤️ᥫ᭡፝֟፝֟† Vol.{Sticker.pack}"
    is_sta = False
    is_ani = False
    is_vid = False
    photo = None
    ...
    if reply and reply.media:
        if reply.photo:
            ...
            if reply.media.document.mime_type == "video/webm":
                await Pbx.edit("__Oow! A video sticker ...__ **[ ENCODING ]**")
                ...
                await Pbx.edit("__Oow! A video ...__ **[ CONVERTING ]**")
                await VSticker(event, reply)
            is_vid = True
            photo = 1
        else:
            return await eod(Pbx, "__Can't kang that 🔪__")
    else:
        return await eod(Pbx, "__Can't kang that 🔪__")
    await Pbx.edit("__**Adding this sticker to your pack...**__")
        ...
        async with event.client.conversation("@Stickers") as conv:
            try:
                ...
                while ("50" in resp.message) or ("120" in resp.message) or is_vid:
                    ...
                    packname = f"PbxBot_{name}_{Sticker.pack}"
                    packnick = f"{custompack}" if custompack else f"{nick}'s 🕊️⃝‌ᴘʙx ❤️ᥫ᭡፝֟፝֟† Vol.{Sticker.pack}"
                    await Pbx.edit(f"__**Switching pack due to insufficient space ...**__ \n__Pack:__ `{Sticker.pack}`")
                    await conv.send_message(packname)
                    resp = await conv.get_response()
                    if resp.message == "Invalid set selected.":
                        ...
                        if "Sorry, the file type is invalid." in rsp.text:
                            return await eod(Pbx, "`Failed to add sticker, use` @Stickers `bot to add the sticker manually.`")
                ...
                return await parse_error(Pbx, "__Unblock @Stickers and try again.__", False)
    else:
        await Pbx.edit("__**Preparing a new pack....**__")
        async with event.client.conversation("@Stickers") as conv:
            await conv.send_message(cmd)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
    ...
    await tbot.send_message(
        Config.LOGGER_ID,
        f"#KANG #STICKER \n\nA sticker has been kanged into the pack of {PBX_USER}. Click below to see the pack!",
        buttons=[
          [Button.url("View Pack", f"t.me/addstickers/{packname}")],
          [Button.url(PBX_USER, f"tg://user?id={II_BAD_BBY_II}")],
        ],
        parse_mode=None,
    )
    await eod(
...
@Pbx_cmd(pattern="pkang(?:\s|$)([\s\S]*)")
async def _(event):
    Pbx = await eor(event, "`Preparing pack kang...`")
    reply = await event.get_reply_message()
    II_BAD_BBY_II, PBX_USER, _ = await client_id(event)
    ...
    user = await event.client.get_me()
    un = f"@{user.username}" if user.username else PBX_USER
    un_ = user.username if user.username else II_BAD_BBY_II
    if not reply:
        return await eod(Pbx, "`Reply to a stciker to kang that pack.`")
    if len(lists) == 1:
        pname = f"{un}'s 🕊️⃝‌ᴘʙx ❤️ᥫ᭡፝֟፝֟† Pack"
    else:
        pname = lists[1].strip()
    if reply and reply.media and reply.media.document.mime_type == "image/webp":
        Pbx_id = reply.media.document.attributes[1].stickerset.id
        Pbx_hash = reply.media.document.attributes[1].stickerset.access_hash
        got_stcr = await event.client(
            functions.messages.GetStickerSetRequest(
                stickerset=types.InputStickerSetID(id=Pbx_id, access_hash=Pbx_hash)
            )
        )
        stcrs = []
        ...
        if x is None:
            ...
            pack = int(x) + 1
        await Pbx.edit("`Starting kang process...`")
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=II_BAD_BBY_II,
                    title=pname,
                    short_name=f"Pbx_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
            addgvar("PKANG", pack)
            ...
            await Pbx.edit("`Pack name already occupied... making new pack`")
            pack = int(pack) + 1
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=II_BAD_BBY_II,
        ...
        await tbot.send_message(
            Config.LOGGER_ID,
            f"#PKANG #STICKER \n\nA sticker pack has been kanged by {PBX_USER}. Click below to see the pack!",
            buttons=[
        ...
        await Pbx.edit("Unsupported File. Please Reply to a sticker only.")
    await to_del.delete()


@Pbx_cmd(pattern="stkrinfo$")
async def get_pack_info(event):
    ...
    try:
        ...
        Pbx = await eor(event, "`Fetching details of the sticker pack, please wait..`")
    ...
    await Pbx.edit(OUTPUT, parse_mode='HTML')


@Pbx_cmd(pattern="delst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
        return await eod(event, "Reply to a sticker to delete it.")
    Pbx = await eor(event, "🥴 `Deleting sticker...`")
    async with event.client.conversation("@Stickers") as conv:
        try:
            ...
            return await parse_error(Pbx, "__Unblock @Stickers and try again.__", False)
        await Pbx.edit(fourth.text)


@Pbx_cmd(pattern="editst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
    ...
    emoji = lists[1].strip()
    Pbx = await eor(event, f"📝 `Editing sticker emoji to {emoji}`")
    async with event.client.conversation("@Stickers") as conv:
        try:
            ...
            if fourth.text.startswith("Current emoji:"):
                ...
                return await eod(Pbx, "That's not your sticker!")
        except YouBlockedUserError:
            return await parse_error(Pbx, "__Unblock @Stickers and try again.__", False)
        await Pbx.edit(f"{sixth.text}")


@Pbx_cmd(pattern="text(?:\s|$)([\s\S]*)")
async def sticklet(event):
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@PbxFonts")
