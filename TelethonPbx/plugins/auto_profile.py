...
@Pbx_cmd(pattern="autoname$")
async def _(event):
    Pbx = await eor(event, "`Starting AutoName Please Wait`")
    _id, PBX_USER, _ment = await client_id(event)
    await Pbx.edit(f"Auto Name has been started my Master")
...
@Pbx_cmd(pattern="autobio$")
async def _(event):
    Pbx = await eor(event, "Starting AutoBio...")
    await Pbx.edit("AutoBio Activated...")
