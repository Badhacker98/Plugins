...
async def _(event):
    ...
    if type_ == 'Sticker':
        Pbx_ = await event.client.download_media(rply.media)
        gifs = await tgs_to_gif(Pbx_, True)
        unsave =  await event.client.send_file(event.chat_id, file=gifs, force_document=False)
        await unsave_gif(event, unsave)
        await Pbx.delete()
        os.remove(Pbx_)
        os.remove("Pbxbot.gif")
    else:
        await parse_error(Pbx, "Only animated stickers are supported.")


@Pbx_cmd(pattern="stoi$")
async def _(Pbx):
    reply_to_id = Pbx.message.id
    if Pbx.reply_to_msg_id:
        reply_to_id = Pbx.reply_to_msg_id
    event = await eor(Pbx, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Pbx.client.download_media(
...
async def _(event):
    ...
    try:
        ...
        await Pbx.edit(f"__Downloaded:__ `{downloaded_file_name}` \n__Time taken:__ `{ms} seconds`")
        ...
        if input_str == "voice":
            ...
            await parse_error(Pbx, "Not supported")
            os.remove(downloaded_file_name)
            return
        ...
        if os.path.exists(new_required_file_name):
            ...
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, Pbx, c_time, "Uploading ...")
                ),
            )
