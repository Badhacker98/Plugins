...
from TelethonPbx.DB import pmpermit_sql as pm_sql
...
if str(Config.PM_PERMIT).lower() not in disabled_list:
    ...
    async def _(event):
        ...
        if str(event.chat_id) == str(II_BAD_BBY_II):
            return
    ...
    async def new_pm(event):
        ...
        PBX_FIRST = f"🔐 𝐏𝐛𝐱𝐛𝐨𝐭 𝐏𝐦 𝐒𝐞𝐜𝐮𝐫𝐢𝐭𝐲 🔐\n\nHello!! This is an automated message on behalf of {Pbx_mention}."
        if CSTM_PMP:
             PBX_FIRST += f"\n\n{CSTM_PMP}"
...
async def do_pm_permit_action(chat_ids, event, client):
    ...
    Pbx = await tap[0].click(event.chat_id)
    client.PM_WARNS[chat_ids] += 1
    if chat_ids in client.PREV_REPLY_MESSAGE:
        await client.PREV_REPLY_MESSAGE[chat_ids].delete()
    client.PREV_REPLY_MESSAGE[chat_ids] = Pbx
...
CmdHelp("pm_permit").add_command(
    ...
    "listapproved", None, "Sends the list of all users approved by 🕊️⃝‌ᴘʙx ❤️ᥫ᭡፝֟፝֟†"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
