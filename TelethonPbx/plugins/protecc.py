...
from TelethonPbx.DB.husb_sql import (add_hus_grp, get_all_hus_grp, is_husb,
                                      rm_hus_grp)
from TelethonPbx.DB.waifu_sql import add_grp, get_all_grp, is_harem, rm_grp
...
@Pbx_cmd(pattern="pt(?:\s|$)([\s\S]*)")
async def _(event):
    BASE_URL = "http://images.google.com"
    if event.reply_to_msg_id:
        Pbx = await eor(event, "Hmm..")
        ...
        await Pbx.edit(f"/protecc {prs_text}", parse_mode="HTML", link_preview=False)
    else:
...
async def _(event):
    ...
    if is_harem(str(event.chat_id)):
        try:
            ...
            alls = div.find("a")
            text = alls.text
            Pbx = await event.client.send_message(
                event.chat_id,
                f"/protecc {text}",
                reply_to=event,
            )
...
@Pbx_cmd(pattern="autowaifu$")
async def _(event):
    Pbx = await eor(event, "Fetching Autowaifu chats...")
    all_grp = get_all_grp()
...
@Pbx_cmd(pattern="autohusbando$")
async def _(event):
    Pbx = await eor(event, "Fetching Autohusbando chats...")
