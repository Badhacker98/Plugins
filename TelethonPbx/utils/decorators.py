...
from TelethonPbx.utils.globals import CMD_LIST, LOAD_PLUG, SUDO_LIST
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                Pbxreg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                Pbxreg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(Pbxreg + pattern)
    ...
    # blacklisted chats.
    # Pbxbot will not respond in these chats.
...
def on(**args):
    def decorator(func):
        ...
        Pbx.add_event_handler(wrapper, events.NewMessage(**args))
        if H2:
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            Pbx.add_event_handler(func, events.MessageEdited(**args))
